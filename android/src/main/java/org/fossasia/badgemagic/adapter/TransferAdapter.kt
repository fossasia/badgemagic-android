...
class TransferAdapter(private val context: Context?, private val list: List<Message>, private val listener: OnTransferItemSelected) : RecyclerView.Adapter<TransferAdapter.TransferItemHolder>() {
    private var selectedPosition: Int = -1

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): TransferItemHolder {
        val v = LayoutInflater.from(context).inflate(R.layout.recycler_transfer_item, parent, false)
        return TransferItemHolder(v)
    }
    ...
    override fun onBindViewHolder(holder: TransferItemHolder, position: Int) {
        holder.bind(list[position])
    }

    override fun getItemCount() = list.size

    fun getSelectedItem(): Message? {
    ...
    inner class TransferItemHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        init {
            ...
            delete.setOnClickListener {
                listener.onDelete(list[adapterPosition])
            }
        }

        fun bind(item: Message) {
            // TODO: Get item name
            // text.text = item.hexStrings.toString()
            text.text = ""
            ...
            val badge: BadgeConfig = JSONHelper.decodeJSON(SendingUtils.configToJSON(item, false))
...
interface OnTransferItemSelected {
    fun onSelected(item: Message?)
    fun onDelete(item: Message)
}
